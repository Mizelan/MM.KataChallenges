using System;
using System.Linq;
using BowlingScoreCalculator.Logics;
using FluentAssertions;
using Xunit;

namespace BowlingScoreCalculator.Unittest
{
    public class FrameMakeTests
    {
        readonly RollRecorder rollRecorder;
        readonly FrameMaker frameMaker;

        public FrameMakeTests()
        {
            rollRecorder = new RollRecorder();
            frameMaker = new FrameMaker(rollRecorder, new GameConfig());
        }

        void PushRolls(params int[] downedPinList)
        {
            foreach (var p in downedPinList)
            {
                rollRecorder.Record(p);
            }
        }

        [Fact]
        public void GetFrames_InProgress()
        {
            PushRolls(1);
            var frames = frameMaker.GetFrames();
            frames.Count().Should().Be(1);
            var frame = frames.First();
            frame.FrameResult.Should().Be(FrameResultType.InProgress);
            frame.Rolls.Select(x => x.DownedPinCount).Should().BeEquivalentTo(new int[] { 1 });
        }

        [Fact]
        public void GetSingleFrame_Open()
        {
            PushRolls(1, 2);
            var frames = frameMaker.GetFrames();
            frames.Count().Should().Be(1);
            var frame = frames.First();
            frame.FrameResult.Should().Be(FrameResultType.Open);
            frame.Rolls.Select(x => x.DownedPinCount).Should().BeEquivalentTo(new int[] { 1, 2});
        }

        [Fact]
        public void GetMultiFrames_Open()
        {
            PushRolls(1, 2, 3, 4);
            var frames = frameMaker.GetFrames();
            frames.Count().Should().Be(2);
            var frame = frames.First();
            frame.FrameResult.Should().Be(FrameResultType.Open);
            frame.Rolls.Select(x => x.DownedPinCount).Should().BeEquivalentTo(new int[] { 1, 2 });
            frame = frames.Skip(1).First();
            frame.FrameResult.Should().Be(FrameResultType.Open);
            frame.Rolls.Select(x => x.DownedPinCount).Should().BeEquivalentTo(new int[] { 3, 4 });
        }

        [Fact]
        public void GetMultiFrames_Strike()
        {
            PushRolls(10, 10, 10);
            var frames = frameMaker.GetFrames();
            frames.Count().Should().Be(3);
            var frame = frames.First();
            frame.FrameResult.Should().Be(FrameResultType.Strike);
            frame.Rolls.Select(x => x.DownedPinCount).Should().BeEquivalentTo(new int[] { 10 });
            frame = frames.Skip(1).First();
            frame.FrameResult.Should().Be(FrameResultType.Strike);
            frame.Rolls.Select(x => x.DownedPinCount).Should().BeEquivalentTo(new int[] { 10 });
            frame = frames.Skip(2).First();
            frame.FrameResult.Should().Be(FrameResultType.Strike);
            frame.Rolls.Select(x => x.DownedPinCount).Should().BeEquivalentTo(new int[] { 10 });
        }

        [Fact]
        public void GetMultiFrames_Spare()
        {
            PushRolls(1, 9, 1, 2);
            var frames = frameMaker.GetFrames();
            frames.Count().Should().Be(2);
            var frame = frames.First();
            frame.FrameResult.Should().Be(FrameResultType.Spare);
            frame.Rolls.Select(x => x.DownedPinCount).Should().BeEquivalentTo(new int[] { 1, 9 });
            frame = frames.Skip(1).First();
            frame.FrameResult.Should().Be(FrameResultType.Open);
            frame.Rolls.Select(x => x.DownedPinCount).Should().BeEquivalentTo(new int[] { 1, 2 });
        }
    }
}
